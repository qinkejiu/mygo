...
// ByteSub_ShiftRow函数
func ByteSub_ShiftRow(statemt []int, nb int) {
	temp := 0
...
// SubByte函数
func SubByte(in int) int {
	return Sbox[in>>4][in&0xf]
}

// InversShiftRow_ByteSub函数
func InversShiftRow_ByteSub(statemt []int, nb int) {
	temp := 0
...
// MixColumn_AddRoundKey函数
func MixColumn_AddRoundKey(statemt []int, nb, n int) int {
	ret := make([]int, 8*4)
	var x int
...
// AddRoundKey_InversMixColumn函数
func AddRoundKey_InversMixColumn(statemt []int, nb, n int) int {
	ret := make([]int, 8*4)
	var x int
...
// AddRoundKey函数
func AddRoundKey(statemt []int, typeAES, n int) int {
	nb := 0

	switch typeAES {
	case 128128, 192128, 256128:
		nb = 4
	case 128192, 192192, 256192:
		nb = 6
	case 128256, 192256, 256256:
		nb = 8
	}
...
// KeySchedule函数
func KeySchedule(typeAES int, key []int) int {
	nk, nb, round_val := 0, 0, 0
	temp := [4]int{}
...
// encrypt函数
func encrypt(statemt, key []int, typeAES int) int {
	outEncStatemt := [16]int{
...
// decrypt函数
func decrypt(statemt, key []int, typeAES int) int {
	...
	AddRoundKey(statemt, typeAES, round_val)
