...
BASE_DIR = SCRIPT_DIR
OUTPUT_FILE = SCRIPT_DIR / "CHS_clean_simulation_results.txt"
...
CASE_COMMAND_TIMEOUT_SECONDS = {
    ("aes", "hardware simulation"): 180,
}
...
def run_command(cmd: str, folder_name: str, cmd_label: str, output_fh) -> bool:
    ...
    try:
        env = prepare_go_env()
        timeout_seconds = CASE_COMMAND_TIMEOUT_SECONDS.get(
            (folder_name, cmd_label), COMMAND_TIMEOUT_SECONDS
        )
        result = subprocess.run(
            cmd,
            shell=True,
        ...
        output_fh.write(f"❌ TIMEOUT ({timeout_seconds}s)\n\n")
        output_fh.flush()
        return False
...
def run_hardware_simulation(folder: str, main_go: str, mygo_bin: str, output_fh) -> bool:
    max_cycles = SIM_MAX_CYCLES.get(folder, 64)
    primary_cmd = f"{mygo_bin} sim --sim-max-cycles {max_cycles} {main_go}"
    return run_command(primary_cmd, folder, "hardware simulation", output_fh)
...
def main():
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        ...
        for idx, folder in enumerate(folders, 1):
            ...
            # 命令1: mygo sim（硬件仿真）
            ok1 = run_hardware_simulation(folder, main_go, mygo_bin, f)
            if ok1:
                success1 += 1
