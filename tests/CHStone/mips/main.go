...
+--------------------------------------------------------------------------+
*/
var A = [8]int32{22, 5, -9, 3, -17, 38, 0, 11}
var outData = [8]int32{-17, -9, 0, 3, 5, 11, 22, 38}

func iaddr(x int32) int32 {
	return (x & 0x000000ff) >> 2
}

func daddr(x int32) int32 {
	return (x & 0x000000ff) >> 2
}
		...
		for pc != 0 {
			...
			case R:
				...
				case SUBU:
					reg[rd] = reg[rs] - reg[rt]
				case MULT:
					hilo = int64(reg[rs]) * int64(reg[rt])
					lo = int32(hilo & 0x00000000ffffffff)
					hi = int32((hilo >> 32) & 0xffffffff)
				case MULTU:
					hilo = int64(uint32(reg[rs])) * int64(uint32(reg[rt]))
					lo = int32(hilo & 0x00000000ffffffff)
					hi = int32((hilo >> 32) & 0xffffffff)
				case MFHI:
					reg[rd] = hi
				...
				case SLL:
					reg[rd] = reg[rt] << shamt
				case SRL:
					reg[rd] = int32(uint32(reg[rt]) >> shamt)
				case SLLV:
					reg[rd] = reg[rt] << uint32(reg[rs])
				case SRLV:
					reg[rd] = int32(uint32(reg[rt]) >> uint32(reg[rs]))
				case SLT:
					if reg[rs] < reg[rt] {
						reg[rd] = 1
					} else {
				...
				case SLTU:
					if uint32(reg[rs]) < uint32(reg[rt]) {
						reg[rd] = 1
					} else {
						reg[rd] = 0
					}
			...
			default:
				address := int16(ins & 0xffff)
				rt := (ins >> 16) & 0x1f
				rs := (ins >> 21) & 0x1f

				switch op {
				case ADDIU:
					reg[rt] = reg[rs] + int32(address)
				case ANDI:
					reg[rt] = reg[rs] & int32(uint16(address))
				case ORI:
					reg[rt] = reg[rs] | int32(uint16(address))
				case XORI:
					reg[rt] = reg[rs] ^ int32(uint16(address))
				case LW:
					reg[rt] = dmem[daddr(reg[rs]+int32(address))]
				case SW:
					dmem[daddr(reg[rs]+int32(address))] = reg[rt]
				case LUI:
					reg[rt] = int32(uint16(address)) << 16
				case BEQ:
					if reg[rs] == reg[rt] {
						pc = pc - 4 + (int32(address) << 2)
					}
				case BNE:
					if reg[rs] != reg[rt] {
						pc = pc - 4 + (int32(address) << 2)
				...
				case SLTI:
					if reg[rs] < int32(address) {
						reg[rt] = 1
					} else {
						reg[rt] = 0
					}
				case SLTIU:
					if uint32(reg[rs]) < uint32(uint16(address)) {
						reg[rt] = 1
					} else {
						reg[rt] = 0
					}
